...
function _AbstractMaskTextArea(fild, regra, tamanho) {
...
function CepMask(fild) {
	...
	if (fild.addEventListener) {
		fild.addEventListener("blur", validaCEP, true);
	} else if (fild.attachEvent) {
		fild.attachEvent("onblur", validaCEP);
	}

	// overriding method
	var desinst = this.desinstalar;
	this.desinstalar = function() {
		if (fild.removeEventListener) {
			fild.removeEventListener("blur", validaCEP, true);
		} else if (fild.detachEvent) {
			fild.detachEvent("onblur", validaCEP);
		}
		desinst();
	};

	var exec = this.executar;
...
function CpfMask(fild) {
	...
	if (fild.addEventListener) {
		fild.addEventListener("blur", validarCpf, true);
	} else if (fild.attachEvent) {
		fild.attachEvent("onblur", validarCpf);
	}
	...
	this.desinstalar = function() {
		if (fild.removeEventListener) {
			fild.removeEventListener("blur", validarCpf, true);
		} else if (fild.detachEvent) {
			fild.detachEvent("onblur", validarCpf);
		}
		desinst();
	};
...
function CnpjMask(fild) {
	...
	if (fild.addEventListener) {
		fild.addEventListener("blur", validarCnpj, true);
	} else if (fild.attachEvent) {
		fild.attachEvent("onblur", validarCnpj);
	}
	...
	this.desinstalar = function() {
		if (fild.removeEventListener) {
			fild.removeEventListener("blur", validarCnpj, true);
		} else if (fild.detachEvent) {
			fild.detachEvent("onblur", validarCnpj);
		}
		desinst();
	};
...
/**
 * Esta classe faz com que o campo receba apenas letras
 */
...
function ChassiMask(fild) {
	...
	_AbstractMask.call(this, fild, chassi);
...
function DataMask(fild) {
	...
	if (fild.addEventListener) {
		fild.addEventListener("blur", validarData, true);
	} else if (fild.attachEvent) {
		fild.attachEvent("onblur", validarData);
	}
	...
	this.desinstalar = function() {
		if (fild.removeEventListener) {
			fild.removeEventListener("blur", validarData, true);
		} else if (fild.detachEvent) {
			fild.detachEvent("onblur", validarData);
		}
		desinst();
	};
 ...
 * @param fild
 */
function AlfaNumericoSemEspaco(fild) {
	// rule
	function alfanum() {
		var v = fild.value;
	...
	_AbstractMaskTextArea.call(this, fild, alfanum);
...
/**
 * mask to allow only letters and especial caracteres
 * 
 * @param fild
 */
function AlfaMask(fild) {
	// rule
	function alfanum() {
		var v = fild.value;
		v = v.replace(/[^\d\w\& ]/g, "");
...
/**
 * mask to allow letters, numbers and especial caracteres
 * 
 * @param fild
 */
function AlfaNumMaskSpecial(fild) {
	// rule
	function alfanum() {
		var v = fild.value;
		v = v.replace(/[^a-zA-Z0-9@;._\&]+/g, "");
		...
		$.fn.mascara = function(p) {
			this.each(function(i, e) {
				...
				case "chassi":
					new ChassiMask(this);
					break;	
				case "alfanumericosemespaco":
					new AlfaNumericoSemEspaco(this);
					break;	
				case "alfa":
					new AlfaMask(this);
					break;	
				case "alfanumericoespecial":
					new AlfaNumMaskSpecial(this);
					break;	
				default:
					throw new Error("the valid masks options are: "//
							+ "'desinstalar','executar','cep','telefone',"//
							+ "'cpf','cnpj','numero',"//
							+ "'leet','valor','data',"//
							+ "'placa','caixaalta', 'alfa', 'alfanumericoespecial'");
				}
				// console.debug(this);
			});
			return this;
